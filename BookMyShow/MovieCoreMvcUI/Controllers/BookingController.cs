...
namespace MovieCoreMvcUI.Controllers
{
    public class BookingController : Controller
    {
        ...
        public async Task<IActionResult> EditBooking(int BookingId)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                string endPoint = _configuration["WebApiBaseUrl"] + "Bookings/GetBookingsById?bookingId=" + BookingId;
                using (var response = await client.GetAsync(endPoint))
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
                    {
        ...
        public async Task<IActionResult> DeleteBooking(Booking booking)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                string endPoint = _configuration["WebApiBaseUrl"] + "Bookings/DeleteBooking?bookingId=" + booking.Id;
